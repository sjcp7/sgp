class StudentTest < ApplicationRecord
  ...
  scope :find_by_student, ->(student) { where(student: student) }

  def update_student_tests
    sq = self.test.school_quarter
    tests = self.student.tests.find_by_lecture(self.test.lecture)
    if self.test.AC?
      update_MAC(sq, tests)
  ...
  def update_MAC(sq, tests)
    mac = tests.MAC.find_by_school_quarter(sq).first.student_tests.find_by_student(self.student).first
    acs = tests.AC.find_by_school_quarter(sq).map do |ac|
      ac_test = ac.student_tests.find_by_student(self.student).first
  ...
  def update_MT(sq, tests)
    mac = tests.MAC.find_by_school_quarter(sq).first.student_tests.find_by_student(self.student).first
    pp = tests.PP.find_by_school_quarter(sq).first.student_tests.find_by_student(self.student).first
    pt = tests.PT.find_by_school_quarter(sq).first.student_tests.find_by_student(self.student).first
    mt = tests.MT.find_by_school_quarter(sq).first.student_tests.find_by_student(self.student).first
  ...
  def update_MT_Technical(p)
    ...
    past_mt = p[:tests].MT.find_by_school_quarter(past_sq).first.student_tests.find_by_student(self.student).first
    average = (past_mt.score + p[:mac].score + p[:pp].score + p[:pt].score) / 4
    p[:mt].update(score: average)
  end

  def update_MAD(sq, tests)
    mad = tests.MAD.first.student_tests.find_by_student(self.student).first
  ...
  def update_MAD_PUNIV(sq, tests, mad)
    if sq.quarter == 3
      mts = tests.MT.map do |mt|
        mt.student_tests.find_by_student(self.student).first.score
      end
  ...
  def update_CF(tests)
    cf = tests.CF.first.student_tests.find_by_student(self.student).first
    mad = tests.MAD.first.student_tests.find_by_student(self.student).first
    ex = tests.EX.first.student_tests.find_by_student(self.student).first
